...
| [bongocat-pet/BongoCat](https://github.com/bongocat-pet/BongoCat)          | 原生窗口、输入队列、Cubism bridge、缩放与托盘职责划分清楚                         | C++ 原生架构不能直接复用；重新实现其整套桥接成本高                                  | 借鉴边界清晰的输入/模型/窗口分层；当前仍保持 Main、Preload、Renderer 安全边界    |
