...
- 小组件行为只能来自仓库内编译的类型化注册器和固定数据源。允许下载经审阅、测量并写入应用信任表的声明式包；包只含受限 JSON 和静态资源，禁止 JS、WASM、DLL、命令、外部脚本及任意包导入、目录扫描发现。
- Main 必须先核对硬编码 SHA256、版本、目标及体积，再执行包内容校验并原子安装。远端只能提供下载路由，不得提供信任依据。首次下载由用户明确点击，安装不自动启用。
- 每个组件声明稳定 ID、最小权限、有限超时、设置视图和失败状态；重复 ID、未知权限和未实现映射必须在构建或启动时失败。
- Renderer 只能提交已批准组件的有限安装请求或组件 ID 与布尔启停状态；Main Process 重新校验并映射到固定能力，不接受 Renderer 提供 URL、命令名、路径、代码、参数列表或可执行入口。
