...
功能提交 [`0c2f819b3ec2e16f6829b5be5a696b69f2c78253`](https://github.com/ph1gros/for-people-no-friend/commit/0c2f819b3ec2e16f6829b5be5a696b69f2c78253) 的 [Windows CI 34706836614](https://github.com/ph1gros/for-people-no-friend/actions/runs/34706836614) 出现 1 项失败、872 项通过。失败位于断点续传测试：服务器固定在 20ms 后断开，而客户端部分文件仍为 0 字节。生产进度通知发生在文件写入完成之后；固定时间不保证 Windows runner 已读取和写入数据，故此结果不是生产续传丢失已确认落盘数据的证明。

修正只改变测试：收到首半块写入进度后再一次性断开服务端响应，明确断言部分文件长度、失败时尚无激活清单、第二次请求恰为对应 Range、最终文件内容和激活记录。没有放宽哈希、内容、原子激活或生产下载实现。该修正作为后续提交单独保留，不覆盖首轮失败历史；最终 CI 绑定 Release 中的完整发布 SHA。
