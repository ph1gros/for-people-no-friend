...
此项不属于下文已发布的 v1.8.1 包。新增独立首次运行向导、服务商连接测试、角色包预览确认、可选语音与资源安装、步骤恢复及托盘重跑入口。升级用户保留既有配置并跳过首次向导；资源按明确选择下载，校验就绪后才启用。DPI、多屏、读屏及真实服务商/资源下载仍需人工验收。范围与验证边界见[向导实现](SETUP_WIZARD_IMPLEMENTATION.md)。
