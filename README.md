...
> **仅限非商业用途。** 本项目原创代码与文档允许在保留署名及声明的前提下，用于非商业使用、学习、修改和分享；未经相关权利人另行书面授权，不得用于收费产品、商业服务、转售或其他盈利、变现用途。详见 [非商业使用许可](LICENSE.md)。第三方代码、模型、音色及角色素材仍须遵守各自的原始许可证与授权条件。
...
从设置或托盘菜单打开“资源中心”。它是独立窗口，支持搜索、分类、安装、暂停、继续和取消；关闭资源窗口不会中止后台下载。主窗口也有任务栏入口，可从最小化状态恢复。

| 想要的功能       | 需要安装的组件                                                 |   下载总量 |
| ---------------- | -------------------------------------------------------------- | ---------: |
| 伊蕾娜日语朗读   | Style-Bert-VITS2 引擎 + 日语 DeBERTa 基础模型 + 伊蕾娜音色模型 | 696.31 MiB |
| 圣园未花日语朗读 | Genie-TTS 引擎 + Genie 基础模型 + 圣园未花（Mika）音色         | 754.64 MiB |
| 菲比中文朗读     | Genie-TTS 引擎 + Genie 基础模型 + 中英文发音词典 + 菲比音色    | 767.03 MiB |
| 37 英语朗读      | Genie-TTS 引擎 + Genie 基础模型 + 中英文发音词典 + 37 音色     | 767.44 MiB |
| 本地麦克风识别   | SenseVoiceSmall 语音识别模型（引擎已内置）                     | 152.88 MiB |

卡片统一显示用途、适配引擎、支持语言、配套资源、版本、体积和许可。基础模型提供文本或参考音频特征，音色模型决定具体角色发声。Mika 为《蔚蓝档案》的圣园未花，支持日语。伊蕾娜仅限非商业用途，分享时须保留随附说明；第三方资源各自的许可证仍然适用。

资源安装后，在语音设置中选择相应引擎。选择 Genie-TTS 会带入本机 Mika 预设；首次启用需要加载模型和预热，完成后复用后台进程。ONNX 原生推理仍需要加载模型，预热期间可以继续使用文字聊天。

启动和刷新目录不会自动下载整套资源。手动安装时显示计费网络状态；下载中变为计费网络或无法确认费用时自动暂停，可手动继续。网络不可用时保留目录和已安装资源。安装使用主程序固定的校验记录，远程目录无法替换信任记录。更新组件可能需要同时更新主程序。

[资源仓库与下载说明](https://github.com/ph1gros/fpnf-resources) · [资源中心开发说明](docs/RESOURCE_CENTER.md) · [Genie 验证记录](docs/GENIE_TTS_HANDOFF.md)

v1.8.2 新增音色：**菲比（Feibi）／中文／《鸣潮》**、**37（ThirtySeven）／英语／《重返未来：1999》**。两者与未花共享 Genie 引擎和基础模型，另需中英文发音词典；语音设置提供中文标注的音色预设，自动配好语言和本机服务地址。菲比直接朗读中文，37 使用当前聊天模型将中文回复转换为英语，英文原文直接朗读。组件通过资源中心按需安装；引擎升级为 1.1.2，包含已试听认可的菲比句尾和 37 短停顿气声处理。安装与验证记录见 [多音色接入记录](docs/GENIE_MULTIVOICE.md)。

非目视验收及组件测量见 [V1.8 交付记录](docs/V1_8_DELIVERY.md)。不同 DPI、多屏、GPU 与软件渲染下的实际布局和语音自然度仍需人工观察，列入 V1.9 验收；自动测试结果不代替这些判断。
...
- **首次设置**：独立向导支持服务商配置与连接测试、角色包预览确认、可选语音资源安装、暂停继续和步骤恢复；已有配置直接进入主界面，托盘可主动重跑。
- **本地语音**：新增菲比中文与 37 英语音色、中英文发音词典；三种 Genie 音色使用独立受管理进程。引擎组件 1.1.2 保留 Mika 结束标记修复，并改善菲比弱尾声与 37 两处短停顿气声。
- **VTube Studio**：改善多语言表情名称和参数特征的映射建议，显示建议来源；未配置任何表情映射时可使用通用参数表达情绪，已有映射的缺项仍明确提示。
- **资源与兼容**：四类十项资源，新版本目录与下载清单独立发布。未变化的组件继续复用原归档；主程序不内置音色权重。向导目前可选 Mika／伊蕾娜，菲比与 37 在主界面语音设置中选择；向导内音色试听尚未提供。
- **验证与限制**：发布测量、实际组件合成与自动检查见 [v1.8.2 交付记录](docs/V1_8_2_DELIVERY.md)。多屏／DPI、读屏及完整桌面目视验收仍保留后续处理。
