...
- 仅用于推理的日语 BERT INT8 模型；
- sherpa-onnx 与 SenseVoiceSmall INT8 中文/日语语音识别模型；
- 对应的开源许可证与模型说明。
...
当前精简组件的 Windows x64 验证包约 672 MiB，不再携带 PyTorch、FunASR、SciPy、
Numba 或训练工具。不安装时，文字聊天、在线或用户自建的 OpenAI 兼容语音
接口、Genie-TTS 和 Fish Audio 仍可使用。组件不会自动下载模型，也不会启动训练。

SenseVoiceSmall 模型与 sherpa-onnx 均声明为 Apache License 2.0；Style-Bert-VITS2
及随附依赖的许可证保留在组件各自目录中。请按各项目许可证使用。
