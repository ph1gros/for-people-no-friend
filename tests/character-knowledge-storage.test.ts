...
describe('V1.2 character knowledge storage', () => {
  ...
  it('persists sources, field paths and evidence basis in SQLite', async () => {
    ...
    const restored = store.get('character-kaltsit');
    expect(restored).toMatchObject({
      schemaVersion: 1,
      characterNamespace: 'character-kaltsit',
      profileRevision: expect.stringMatching(/^lore-v1-[a-f\d]{16}$/),
      sourceWork: '明日方舟 / Arknights',
    });
    expect(restored?.sources).toHaveLength(5);
  ...
  it('replaces one namespace atomically without touching another character', async () => {
    ...
    const other = {
      ...
      records: m3.records.map((record) => ({
        ...record,
        id: record.id.replace('character-kaltsit', 'character-other'),
        characterNamespace: 'character-other',
      })),
    };
    store.replace(m3);
    store.replace(other);
    store.replace({ ...m3, sourceWork: '更新后的明日方舟' });

    expect(store.get('character-kaltsit')?.sourceWork).toBe('更新后的明日方舟');
    expect(store.get('character-other')?.sourceWork).toBe('明日方舟 / Arknights');
  });
  ...
  it('hard-deletes only the requested character namespace and its evidence', async () => {
    ...
    store.replace(base);
    expect(store.clear('character-kaltsit')).toBe(base.records.length);
    expect(store.get('character-kaltsit')).toBeUndefined();
    expect(
      database?.connection
        .prepare(
  ...
  it('soft-degrades a corrupt keyword index instead of returning partial records', async () => {
    ...
    database?.connection
      ...
      .run('character-kaltsit', base.records[0]?.id ?? '');
    expect(store.get('character-kaltsit')).toBeUndefined();
  });
