...
const windowState = vi.hoisted(() => ({ created: [] as unknown[], failLoad: false }));
...
import {
  createSetupWindow,
  SETUP_WINDOW_PARTITION,
  waitForSetupWindowLoad,
} from '../src/main/windows/create-setup-window';
...
describe('setup window creation', () => {
  beforeEach(() => {
    ...
    windowState.failLoad = false;
  });

  it.each([undefined, 'http://127.0.0.1:5173'])(
    'reports async page load failure for %s',
    async (url) => {
      windowState.failLoad = true;
      const window = createSetupWindow(assets, url);
      await expect(waitForSetupWindowLoad(window)).rejects.toThrow('fixture load failure');
    },
  );
